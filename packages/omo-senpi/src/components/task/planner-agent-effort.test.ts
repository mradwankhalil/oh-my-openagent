...
describe("agent plans carrying configured effort", () => {
  test("#given an agent model entry with effort #when planned #then the effort becomes the child variant", () => {
    ...
    const config = {
      agents: {
        explore: {
          models: [{ model: "chatgpt-subscription/gpt-6-luna-fast", reasoningEffort: "minimal" as const }],
        },
      },
    } satisfies OmoConfig
    const agents = mapOmoConfigAgents(config)
    const models = registry([{ provider: "chatgpt-subscription", id: "gpt-6-luna-fast" }])
    ...
    // then
    expect(plan.model).toBe("chatgpt-subscription/gpt-6-luna-fast")
