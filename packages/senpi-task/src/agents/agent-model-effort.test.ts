...
describe("agent model entries carrying effort", () => {
  test("#given an omo agent whose models carry effort #when bridged #then the entry objects survive onto the definition", () => {
    ...
    const config = {
      agents: {
        explore: {
          models: [{ model: "chatgpt-subscription/gpt-5.6-luna-fast", reasoningEffort: "minimal" as const }],
        },
      },
    }
    ...
    expect(entry.model).toBe("chatgpt-subscription/gpt-5.6-luna-fast")
  ...
  test("#given a canonicalized agent entry #when resolved through resolveAgent #then its canonical reasoning reaches the resolved model record", () => {
    ...
    const agents = roster({
      name: "explore",
      models: [{ model: "chatgpt-subscription/gpt-5.6-luna-fast", reasoning: "high" }],
    })
    const models = registry([{ provider: "chatgpt-subscription", id: "gpt-5.6-luna-fast" }])
  ...
  test("#given a resolved agent model entry carrying effort #when resolved #then the effort reaches the resolved model record", () => {
    ...
    const agents = roster({
      name: "explore",
      models: [{ model: "chatgpt-subscription/gpt-5.6-luna-fast", reasoningEffort: "minimal" }],
    })
    const models = registry([{ provider: "chatgpt-subscription", id: "gpt-5.6-luna-fast" }])
    ...
    // then
    expect(result.model).toBe("chatgpt-subscription/gpt-5.6-luna-fast")
  ...
  test("#given an agent whose top level effort applies to its primary model #when resolved #then the effort reaches the record", () => {
    ...
    const agents = roster({
      ...
      models: ["chatgpt-subscription/gpt-5.6-luna-fast"],
    })
    const models = registry([{ provider: "chatgpt-subscription", id: "gpt-5.6-luna-fast" }])
